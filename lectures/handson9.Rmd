...
1.2 Open `Rstudio` (or `R`) on your local computer. If your `Rstudio` (or `R`) is already open, restart it to have a new work environment to start with with no packages / variables loaded.

If your `Rstudio` is already open, go to *Session* on the top menu, and then click *Restart R*.

From here, All analysis steps will be conducted in R environment on local computers. 
...
```{R, echo=TRUE, eval=TRUE}
## replace [path] with your own project directory path
## on windows machines, replace [\] with [/]
path <- 'C:/Users/rbaoc/Desktop/class/MSIB32500/lectures'
...
cpan.pkg.list <- c('ggplot2', 'RColorBrewer')
bioc.pkg.list <- c('edgeR', 'DESeq2', 
                   'vsn',
                  'genefilter', 'pheatmap', 
...
If promoted to answer `Update all/some/none? [a/s/n]:`, try `n` (none).
...
## 4 Homework (due next Saturday 06/08/2019 at 11:59 PM)
